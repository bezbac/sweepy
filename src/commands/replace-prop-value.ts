...
const prepareReplacement = (options: ReplacePropValueOptions) =>
  Effect.gen(function* () {
    ...
    const { replacedUsages, unsupported } = yield* Effect.try({
      try: () => {
        ...
        for (const sourceFile of sourceFiles) {
          ...
          for (const element of openingElements) {
            ...
            if (targetAttribute !== undefined) {
              const actualTargetValue =
                getStaticAttributeValue(targetAttribute);
              if (
...
export const replacePropValue = (options: ReplacePropValueOptions) =>
  Effect.gen(function* () {
    const { changedFiles, replacedUsages, unsupported } =
      yield* prepareReplacement(options);
